...
async function foodApi() {
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/list.php?a=list`)
...
function displayapi() {
    ...
    for (let i = 0; i < container.length; i++) {
        ...
        <div onclick="giveArea('${container[i].strArea}')"  class="item position-relative overflow-hidden rounded">
            
                <div class="">
                    <i class="fa-solid fa-city fa-3x text-danger"></i>
                    <h2 class="text-white">${container[i].strArea }</h2>
                </div>
...
async function giveArea(term) {
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${term}`)
    let resault = await respond.json()
    container = resault.meals
    console.log(container);
    displayapiarea()
