...
async function foodApi() {
    ...
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/list.php?i=list`)
...
function displayapi() {
    ...
    for (var i = 0; i < 20 ; i++) {
        ...
        <div onclick="giveIngrdients('${container[i].strIngredient}')" class="item position-relative overflow-hidden rounded">
            
                <div class="">
                    <i class="fa-solid fa-bowl-food fa-3x text-success"></i>
                    <h2 class="text-white">${container[i].strIngredient}</h2>
                    <p class="text-white">${container[i].strDescription.slice(0,110)}</p>
                </div>
            
            
        </div>
    </div>`
    console.log(container[i].strDescription);
    }
    document.querySelector('.show').innerHTML = html ;


}
async function giveIngrdients(term) {
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${term}`)
    let resault = await respond.json()
    container = resault.meals
    console.log(container);
    displayapiingrdient()
