...
let container =[];
async function foodApi(term) {
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${term}`)
...
async function foodApifletter(term) {
    let respond = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${term}`)
    let resault = await respond.json()
    container = resault.meals
